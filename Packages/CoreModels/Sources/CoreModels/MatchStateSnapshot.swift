...
public struct MatchStateSnapshot: Codable, Sendable {
    ...
    private func upsertPlayer(_ snapshot: PlayerSnapshot, in context: ModelContext) -> Player {
        ...
        if let existing = try? context.fetch(descriptor).first {
            // Yalnızca YÜKSELT: gelen ad gerçekse ve yereldeki ad boş/placeholder
            // ise güncelle. Böylece rakip gerçek ismini set edince sende de
            // düzelir; kendi gerçek adın ise rakibin bayat snapshot'ıyla
            // placeholder'a düşmez.
            let incoming = snapshot.displayName.trimmingCharacters(in: .whitespacesAndNewlines)
            if Player.isRealName(incoming),
               existing.displayName.isEmpty || Player.isAutoGeneratedName(existing.displayName) {
                existing.displayName = incoming
            }
            return existing
        }
